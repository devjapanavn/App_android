...
class UploadImages{
    ...
    public function upload($image_loc,$new_loc,$width = 3000,$height = 3000,$method = "max"){
        $image_loc = str_replace(URL, $_SERVER['DOCUMENT_ROOT']."/", $image_loc);
        ...
        if (!$image_loc) { $GLOBALS['errors'][] = 'No source image location specified.'; }
            ...
            if (!in_array($extension,array('.jpg','.jpeg','.png','.gif','.bmp','.JPG','.JPEG','.PNG','.GIF','.BMP'))) { $GLOBALS['errors'][] = 'Invalid source file extension!'; }
        }
        if (!$new_loc) { $GLOBALS['errors'][] = 'No destination image location specified.'; }
        else {
            $new_extension = strtolower(substr($new_loc,strrpos($new_loc,'.')));
            if (!in_array($new_extension,array('.jpg','.jpeg','.png','.gif','.bmp','.JPG','.JPEG','.PNG','.GIF','.BMP'))) { $GLOBALS['errors'][] = 'Invalid destination file extension!'; }
        }
        ...
        elseif ($extension == '.png') { imagepng($new_image,$new_loc,9) or ($save_error = true); }
    ...
    public function checkImageMulti($file){
        $array = array();
        if(isset($file['name']) && $file['name'] != ''){
            if(!$this->checkfile($file['type'])){
                $array["message"] = "Ảnh: ko đúng định dạng";
            }
            if($file['size'] > 20480000){
                $array["message"] = "Kích Thước Ảnh: phải nhỏ hơn 2MB";
            }
            $toSlug = new ToSlug();
            $array["images"] = time()."-".$toSlug->nameImage($file['name'], 50);
        }
        return $array;
    }
    ...
    public function checkImage($images){
        ...
        if(isset($images['name']) && $images['name'] != ''){
            if(!$this->checkfile($images['type'])){
                $array["message"] = "Ảnh: ko đúng định dạng";
            }elseif($images['size'] > 20480000){
                $array["message"] = "Kích Thước Ảnh: phải nhỏ hơn 20MB";
            }else{
                $toSlug = new ToSlug();
                $array["images"] = time()."-".$toSlug->nameImage($images['name'], 50);
            }
        }
        return $array;
    }
    
    public function uploadFtp($localfile = ''){
        $conn_id = ftp_connect(FTP_SERVER, 21, 30);
        if($conn_id){
            $login_result = ftp_login($conn_id,FTP_SERVER, FTP_PASS);
        }
        $filesave = FTP_FOLDER.$localfile;
        ftp_put($conn_id, $filesave, $localfile, FTP_BINARY);
        unlink($localfile);
    }
    
    private function checkfile($file_type){
    ...
    private function resize_image_max($image,$max_width,$max_height) {
        $w = imagesx($image);
        $h = imagesy($image);
        if ((!$w) || (!$h)) { $GLOBALS['errors'][] = 'Image couldn\'t be resized because it wasn\'t a valid image.'; return false; }
