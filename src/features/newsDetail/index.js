...
const Screen = props => {
  ...
  const {status, data, error, refetch} = useQuery(
    ['getDetailNews', route.params?.id],
    () => fetch(route.params?.id),
  );
  return (
    <BodyComponent style={styles.box} showBack={true}>
      <Text  h4>{data?.info?.name}</Text>
      <RenderHtml
        source={{html: data?.info?.desc}}
...
export const NewsDetailScreen = Screen;
